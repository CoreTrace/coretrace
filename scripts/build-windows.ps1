...
{
    $cmakeArgs += "-DCORETRACE_VERSION_OVERRIDE=$VersionOverride"
}
...
$binaryPath = Join-Path $resolvedInstallDir "bin\ctrace.exe"
...
if ($PackageZip)
{
    $zipPath = Join-Path $repoRoot "coretrace-windows-$Configuration.zip"
