...
protected:
    ...
    void run() override
    {
        ...
        if (posix_spawn_file_actions_init(&file_actions) != 0) {
            throw std::runtime_error("Failed to init file actions: " + std::string(strerror(errno)));
        }
        ...
        if (posix_spawn_file_actions_adddup2(&file_actions, log_fd_, STDOUT_FILENO) != 0 ||
            ...
            throw std::runtime_error("Failed to setup file redirection: " + std::string(strerror(errno)));
        }
        ...
        int status = posix_spawn(&pid_, resolved_path_.c_str(), &file_actions, &attr,
                               argv.data(), environ);
    ...
    void prepareArguments() override {
        ...
        m_arguments.insert(m_arguments.end(),
                       additional_args_.begin(),
                       additional_args_.end());
    }
...
private:
    void setupLogging() {
        ...
        if (log_fd_ == -1) {
            throw std::runtime_error("Failed to create temp log file: " + std::string(strerror(errno)));
        }
        unlink(temp_path);
    }
    ...
    void checkPermissions() {
        if (access(resolved_path_.c_str(), X_OK) != 0) {
            throw std::runtime_error("Command not executable: " + resolved_path_ + ": " + std::string(strerror(errno)));
        }

        struct stat st;
        if (stat(resolved_path_.c_str(), &st) != 0) {
            throw std::runtime_error("Cannot stat command: " + resolved_path_ + ": " + std::string(strerror(errno)));
        }
