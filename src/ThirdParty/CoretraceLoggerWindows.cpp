...
namespace coretrace
{
    namespace
    {
        ...
        bool g_min_level_set_explicitly = false;
        bool g_modules_set_explicitly = false;
        constexpr int kMaxModules = 32;
        std::string g_modules[kMaxModules];
        int g_module_count = 0;
        bool g_module_filter_active = false;

        [[nodiscard]] bool asciiCaseEqual(std::string_view lhs, std::string_view rhs)
        {
            ...
            {
                const char left = static_cast<char>(std::tolower(static_cast<unsigned char>(lhs[i])));
                const char right =
        ...
        [[nodiscard]] std::string current_prefix()
        {
        ...
        [[nodiscard]] int parse_level_from_env(const char* value)
        {
            ...
            if (asciiCaseEqual(value, "debug"))
            {
                return static_cast<int>(Level::Debug);
            }
            if (asciiCaseEqual(value, "warn"))
            {
                return static_cast<int>(Level::Warn);
            }
            if (asciiCaseEqual(value, "error"))
            {
        ...
        void init_from_env()
        {
            ...
            if (!g_modules_set_explicitly)
            {
                ...
                while (*start != '\0')
                {
                    ...
                    if (end > start)
                    {
                        add_module_locked(std::string_view(start, static_cast<std::size_t>(end - start)));
                    }
                    start = (*end == ',') ? end + 1 : end;
                }
            }
        ...
        [[nodiscard]] std::string make_timestamp_prefix()
        {
            ...
            std::snprintf(buffer, sizeof(buffer), "[%04d-%02d-%02dT%02d:%02d:%02d.%03d] ",
                          ...
                          tmBuf.tm_min, tmBuf.tm_sec, static_cast<int>(millis.count()));
            return buffer;
        }
    ...
    {
        g_min_level_set_explicitly = true;
        init_once();
    ...
    {
        write_str(std::to_string(value));
    }

    void write_hex(uintptr_t value)
    {
        write_str(std::format("0x{:x}", value));
    }
    ...
    {
        const std::string prefix = current_prefix();
    ...
    void write_log_line(Level level, std::string_view module, std::string_view message,
